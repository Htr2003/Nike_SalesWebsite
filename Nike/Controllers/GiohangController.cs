...
namespace Nike.Controllers
{
    public class GiohangController : Controller
    {
        ...
        {
            var dsProduct = (from s in _db.Product select s).ToList();
            //Thêm đơn hàng
            Order order = new Order();
            KhachHang khsession = (KhachHang)Session["Taikhoan"];
            KhachHang kh = _db.KhachHang.Find(khsession.idUser);
        ...
        public ActionResult Payment() // tạo yêu cầu thanh toán và chuyển hướng người dùng đến trang thanh toán
        {
            ...
            pay.AddRequestData("vnp_TxnRef", _db.Order.ToList().LastOrDefault().ID.ToString()+1); //mã hóa đơn
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00")
                    {
                        ...
                        foreach (var item in gh)
                        {
                            ...
                            ctdh.SoLuong = item.SoLuong;
                            ctdh.Price = item.Price;
                            Product product = _db.Product.ToList().Find(n => n.Id == item.IdProduct);
                            product.SoLuong -= 1;
